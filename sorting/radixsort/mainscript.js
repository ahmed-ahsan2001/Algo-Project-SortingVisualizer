...
function smoothAnimation(arr, i1, i2) {
  ...
  function forLoop(i, index) {
    ctx.fillStyle = isRainbowModeOn ? arr[index][1] : "yellow";
    ...
    if (isStapleWideEnoughForNumberDisplay)
      applyText(arr[index][0].toString(), i, height - arr[index][0], "yellow");
    if (index < i2)
...
function heapSort(arr) {
  ...
  function heapifyElement(arr, n, idx) {
    ...
    if (l < n && arr[largestIndex] < arr[l]) {
      frameSaveArray.push(
        frameDuoColorChanger(arr, l, largestIndex, "yellow", true)
      );
      largestIndex = l;
    }
  ...
  frameSaveArray.push(frameDuoColorChanger(arr, 0, 0, "green", false));
  if (isRainbowModeOn) rainbowSortStabilityCorrection();
  frameSaveArray.push(frameDuoColorChanger(arr, 0, 0, "green", false));
}
...
function bubbleSort(arr) {
  ...
  while (!sorted && 0 < unsortedLen--) {
    ...
    for (let i = 0; i < unsortedLen; i++) {
      frameSaveArray.push(frameDuoColorChanger(arr, i, i + 1, "yellow", true));
      if (arr[i + 1] < arr[i]) {
    ...
    frameSaveArray.push(
      frameDuoColorChanger(arr, unsortedLen, unsortedLen, "green", false)
    );
